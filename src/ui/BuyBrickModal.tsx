...
export default function BuyBrickModal({
  ...
  const handlePaymentTokenChange = (token: "SOL" | "USDC") => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (form.is_gift && !form.recipient_email?.trim()) {
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-600 sm:text-sm">
              Pay with SOL or USDC on Solana, then claim your spot on the wall.
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              <button
                type="button"
                onClick={() => handlePaymentTokenChange("SOL")}
                className={`flex items-center gap-2 rounded-full border px-3 py-1.5 font-semibold shadow-sm ${
                  form.paymentToken === "SOL"
              ...
              <button
                type="button"
                onClick={() => handlePaymentTokenChange("USDC")}
                className={`flex items-center gap-2 rounded-full border px-3 py-1.5 font-semibold shadow-sm ${
            ...
            <p className="mt-1 text-[11px] text-slate-500">
              You&apos;ll be asked to approve a Solana transaction in your wallet.
            </p>
          </div>
          ...
          <div className="mt-2 flex flex-wrap justify-end gap-2">
            ...
            >
              {submitting
                ? "Processing..."
                : form.is_gift
                ? `Pay with ${form.paymentToken} & send gift`
                : `Pay with ${form.paymentToken}`}
            </button>
          </div>
        </form>
      </div>
